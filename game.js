// game.js - VERSIÓN CORREGIDA PARA GITHUB PAGES
...
document.addEventListener("DOMContentLoaded", () => {
   ...
   // ======================================================
    ...
    // Función segura para reproducir (evita que el juego se rompa si falla el audio)
    function safePlay(key) {
        if (isMuted || !audioCtx[key]) return;

        const sound = audioCtx[key];
        
        // Si es efecto de sonido (no música), clonamos para permitir sonidos simultáneos
        if (!key.includes('Music')) {
            const clone = sound.cloneNode();
            clone.volume = sound.volume;
            clone.play().catch(e => {
                // Ignoramos errores de "Autoplay" en efectos secundarios, solo avisamos en consola
                console.warn(`⚠️ No se pudo reproducir SFX '${key}':`, e.message);
            });
        } else {
            // Es música
            ...
            if (playPromise !== undefined) {
                playPromise.catch(error => {
                    if (error.name === "NotAllowedError") {
                        console.warn(`🛑 Autoplay bloqueado para '${key}'. Esperando clic del usuario.`);
                    } else {
    ...
    function stopAllMusic() {
        Object.keys(audioCtx).forEach(key => {
            if (key.includes('Music') || key === 'gameover' || key === 'win') {
                audioCtx[key].pause();
                audioCtx[key].currentTime = 0;
            }
        });
    }
    ...
    function cambiarMusica(tipo) {
        ...
        // Detener músicas anteriores
        if (audioCtx.menuMusic) { audioCtx.menuMusic.pause(); audioCtx.menuMusic.currentTime = 0; }
        if (audioCtx.gameMusic) { audioCtx.gameMusic.pause(); audioCtx.gameMusic.currentTime = 0; }

        if (tipo === "menu") safePlay("menuMusic");
        if (tipo === "juego") safePlay("gameMusic");
    }
    ...
    function startGame(level) {
      // Limpiamos sonidos de gameover/win por seguridad
      if(audioCtx.gameover) { audioCtx.gameover.pause(); audioCtx.gameover.currentTime = 0; }
      if(audioCtx.win) { audioCtx.win.pause(); audioCtx.win.currentTime = 0; }
    ...
    function checkWin() {
      ...
      if (tocaArbol) {
        if (applesCollected < totalApples) return; 
    ...
    function togglePause() {
      ...
      if (isPaused) {
        ...
        stopAndRestartTimer(1000 / timerSlowedFactor);
        if(!isMuted && audioCtx.gameMusic) safePlay("gameMusic");
      }
    }
    ...
    function actualizarVolumen(valor) {
      ...
      Object.keys(audioCtx).forEach(key => {
          if (audioCtx[key]) {
             // La música de fondo usualmente se quiere más baja que los efectos
             if(key.includes('Music')) audioCtx[key].volume = Math.min(1.0, vol * 0.3); // Tope 30% para música
             else audioCtx[key].volume = vol;
          }
      });
    }
    ...
    // --- AUTOPLAY FIX (IMPORTANTE PARA GITHUB PAGES) ---
    // El navegador necesita un primer clic para permitir reproducir música.
    document.body.addEventListener('click', () => {
      if (!gameActive && (!audioCtx.menuMusic || audioCtx.menuMusic.paused) && !isMuted) {
         cambiarMusica("menu");
      }
    }, { once: true });
